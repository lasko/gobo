...
- **In-game chat (opt-in, read-only):** off by default to keep the connect minimal — `game/connect` sends `chat:false` and no chat flows. The `SettingsStore.chatEnabled` toggle flips `game/connect` to `chat:true`, after which the server pushes `game/<id>/chat` events: `{type, message:{username, player_id, date, body}}` where `body` is text **or** an analysis/variation object (skipped — read-only renders text only). Parsed by `parseGameChat` in `net/GameChat.kt`; `GameViewModel` collects them into `chat`. **Sending is not implemented:** `game/chat` requires a separate `game_chat_auth` token (not `user_jwt`) plus `username`/`ranking`/`ui_class`/`is_player`, none of which we capture yet — verify against a live game before adding.
...
- `net/GameChatTest` — `parseGameChat`: plain-text line, `message`/`line` key fallback, skipping analysis bodies, defaults, and the no-message case.
