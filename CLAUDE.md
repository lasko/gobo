...
- **Pending sent challenges:** withdraw with `DELETE /api/v1/me/challenges/{id}` and list with `GET /api/v1/me/challenges` (`parseSentChallenges` filters to ones *you* sent); My Games shows them as a cancellable **Pending** section. **Caveat (verified live):** an **open seek does not persist** unless the creator keeps it alive — the web client sends periodic `challenge/keepalive` socket messages after posting, and without them OGS expires the seek within seconds, so it never appears in `/me/challenges`. We don't send the keepalive yet, so the Pending section stays empty for open seeks (tracked separately). The endpoint/parse shapes are otherwise correct (`/me/challenges` returns a paginated `{count,results:[…]}`).
