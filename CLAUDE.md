...
- `net/GameChatTest` — `parseGameChat` (plain-text line, `message`/`line` key fallback, skipping analysis bodies, defaults, no-message case) and `buildGameChatMessage` (send payload shape, no auth/identity fields).
