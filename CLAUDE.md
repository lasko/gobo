...
- `net/GameChatTest` — `parseGameChat` (plain-text line, `message`/`line` key fallback, skipping analysis bodies, defaults, no-message case, `chat_id`/composite identity) and `buildGameChatMessage` (send payload shape, no auth/identity fields), plus `appendChat` idempotency (re-sent log doesn't duplicate).
